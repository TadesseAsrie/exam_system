  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="glass-card rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Timer duration={currentExam.duration} onTimeUp={handleTimeUp} isActive={!timeUp} />
        <button
          onClick={handleSubmit}
          disabled={isSubmitting}
          className="btn-primary flex items-center gap-2 disabled:opacity-50"
        ...
        <div className="lg:col-span-3">
          ...
          <div className="flex justify-between mt-4">
            ...
            {currentIndex === currentQuestions.length - 1 ? (
              ...
              <button
                onClick={() => setCurrentIndex((prev) => prev + 1)}
                disabled={!selected}   // <--- THIS IS THE NEW CHANGE
                className="btn-primary flex items-center gap-2 disabled:opacity-50"
              >
                Next <FiChevronRight />
              </button>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          <div className="grid grid-cols-5 gap-2">
            {currentQuestions.map((q, idx) => {
              ...
              return (
                ...
                >
                  {idx + 1}
                  {isAnswered && <FiCheckCircle size={12} className="inline ml-1" />}
                </button>
              );
            })}
          </div>
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-2">
                <FiCircle />
                <span>Remaining: {currentQuestions.length - answeredCount}</span>
              </div>
            </div>
          </div>
        </div>
